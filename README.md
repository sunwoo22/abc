...
객체지향이란 현실에 존재하는 사물을 있는 그대로 모델링하여, 이들의 **행위와 속성을 정의**하고, 절차적이 아닌 객체가 중심이 되어 **실제 사물이 동작하는 방식으로 설계**하는 것이다. 이는 사물에 대해 <b>객체(Object)</b>라고 부르며, 해당 사물이 하는 **행위를 Method**로 정의하고 해당 사물이 가지는 속성을 <b>변수(Variable)</b>라고 정의한다. 실제 사물을 중심으로 설계하기 때문에 기존의 절차지향보다는 조금 더 **편리한 설계**가 가능해졌다.   
